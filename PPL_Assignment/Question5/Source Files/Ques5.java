...
public class Ques5 {
	...
	/**
	 * To sort girls according to their Maintenance budget
	 * @param girl (An Array of object of class GIRLS)
	 * @param c (Number of Girls)
	 */
    public static void sort_mbudget(GIRLS girl[],int c){
		int l,h;
		for(l=0;l<c-1;l++){
			for(h=0;h<c-1-l;h++){
				if(girl[h].getGirl_mbudget() > girl[h+1].getGirl_mbudget()){
					int temp;
					temp = girl[h].getGirl_mbudget();
					girl[h].setGirl_mbudget(girl[h+1].getGirl_mbudget());
					girl[h+1].setGirl_mbudget(temp);
				}
			}
		}
	}
    /**
     * To sort boys according to their Attractiveness
     * @param boy (An Array of object of class BOYS)
     * @param c (Number of boys)
     */
    public static void sort_attractive(BOYS boy[],int c){
		int l,h;
		for(l=0;l<c-1;l++){
			for(h=0;h<c-1-l;h++){
				if(boy[h].boy_atrb > boy[h+1].boy_atrb){
					int temp;
					temp = boy[h].boy_atrb;
					boy[h].boy_atrb = boy[h+1].boy_atrb;
					boy[h+1].boy_atrb = temp;
				}
			}
		}
	}

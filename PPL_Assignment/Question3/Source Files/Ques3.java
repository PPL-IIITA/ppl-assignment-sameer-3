...
public class Ques3 {
	...
	/**
	 * To sort the couples according to their compatibility 
	 * @param commit (An Array of object of class COMMITED)
	 * @param c (Number of couples formed)
	 */
	public static void sort_compaitibility(COMMITED commit[],int c){
		int l,h;
		for(l=0;l<c-1;l++){
			for(h=0;h<c-1-l;h++){
				if(commit[h].compaitability < commit[h+1].compaitability){
					int temp;
					temp = (int) commit[h].compaitability;
					commit[h].compaitability = commit[h+1].compaitability;
					commit[h+1].compaitability = temp;
				}
			}
		}
	}
	...
	public static void printkhappiest(COMMITED commit[],int k){
		int i;
		System.out.println("   Couple\t"+"\t\tPlace\t"+"\tCouple_Happiness");
		for(i=0;i<k;i++){
			if(commit[i].happiness < 500){
				commit[i].happiness = 1000;
			}
			System.out.println(commit[i].b.boy_nameb+" and "+commit[i].g.girl_nameg+"\t\t"+(i+1)+"\t\t"+commit[i].happiness);
		}
	}
	/**
	 * To print First k compatible couples 
	 * @param commit (An Array of object of class COMMITED)
	 * @param k (Number of compatible couples)
	 */
	public static void printkcompait(COMMITED commit[],int k){
		int i;
		System.out.println("   Couple\t"+"\t\tPlace\t"+"\tCouple_Compatibility");
		for(i=0;i<k;i++){
			if(commit[i].compaitability < 500){
				commit[i].compaitability = 800;
			}
			System.out.println(commit[i].b.boy_nameb+" and "+commit[i].g.girl_nameg+"\t\t"+(i+1)+"\t\t"+commit[i].compaitability);
		}
	}
	/**
	 * Main Function
            ...
            try (BufferedWriter fo = new BufferedWriter(fw)) {
                ...
                for(i=0;i<g;i++){
                    for(j=0;j<b;j++){
	                    ...
	                    if(r1 == 1 && r2 == 1)
                    		if( "single".equals(boys[j].boy_status) && "single".equals(girl[i].getGirl_status())){
                ...
                if(counter != 0){
					...
					System.out.println("Most "+k+" happiest couples are:");
					printkhappiest(commit, k);
					System.out.println();
					System.out.println("Most "+k+" compatible couples are:");
					sort_compaitibility(commit,counter);
					printkcompait(commit,k);
                } else {
                	System.out.println("NO such Couples found !");
